...
public class FileDownload extends HttpServlet {
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		// 업로드된 파일들이 저장된 폴더 설정
	     ...
	     // 다운 받을 파일의 정보를 갖는 File객체생성 ==> 실제 저장된 파일명을 지정한다.
	     File downFile = new File(f, fvo.getSave_file_name());
	     
	     if(downFile.exists()) {  // 파일이 있을 때 작업해야 한다.
	    	 // contentType 설정
	    	 //response.setContentType("text/html; chatset=utf-8");  // html
	    	 //response.setContentType("application/json; chatset=utf-8"); // json
	    	 response.setContentType("application/octet-stream; chatset=utf-8"); // 파일 
