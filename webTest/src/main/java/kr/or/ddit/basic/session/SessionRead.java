...
public class SessionRead extends HttpServlet {
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
	    ...
	    out.println("<h2>저장된 Session 데이터 확인하기</h><hr>");
	    
	    out.println("<h3>Session 데이터 1개 확인하기</h3>");
	    // 2. 저장된 Session데이터를 개별적으로 가져올 때는 getAttribute()메서드를 사용한다.
	    // 형식) session객체.getAttribute("key값");
	    String sessionaValue = (String)session.getAttribute("testSession");
	    if(sessionaValue==null) {
	    	out.println("<h3>'testSession'의 세션값이 없습니다. </h3>");
	    }else {
	    	out.println("<h3>'testSession : " + sessionaValue + "</h3>");	    	
	    }
	    ...
	    out.println("<br><br>");
		out.println("<a href='" + request.getContextPath()+"/basic/session/sessionTest.jsp'>시작문서로 이동하기</a><br><br>");
